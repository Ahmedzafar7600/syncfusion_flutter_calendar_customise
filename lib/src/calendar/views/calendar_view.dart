...
class _CalendarViewState extends State<_CalendarView>
  ...
  getTimeSlotLabel(){
    ...
    DateTime startDateTime=DateTime(currentDateTime.year,currentDateTime.month,currentDateTime.day,widget.calendar.timeSlotViewSettings.startHour.toInt());
    DateTime endDateTime=DateTime(currentDateTime.year,currentDateTime.month,currentDateTime.day,widget.calendar.timeSlotViewSettings.endHour.toInt());
    for(DateTime i=startDateTime;i.hour<=endDateTime.hour;i.add(Duration(hours: 1))){
      list.add(Text("${i.hour}"));
    }
    return list;
  }
...
class _ViewHeaderViewPainter extends CustomPainter {
  ...
  void _addViewHeaderForMonthView(
   ...
   // TextStyle dayTextStyle = getColorForDayInMonthView(today,viewHeaderDayStyle);
    ...
    for (int i = 0; i < DateTime.daysPerWeek; i++) {
      ...
      if (hasToday && currentDate.weekday == today.weekday) {
        ...
        dayTextStyle = todayTextStyle != null
            ? todayTextStyle!.copyWith(
                fontSize: viewHeaderDayStyle.fontSize, color: Colors.black)
            : viewHeaderDayStyle.copyWith(color: Colors.black);
      } else {
       // dayTextStyle = viewHeaderDayStyle;
        dayTextStyle = getColorForDayInMonthView(visibleDates[i],viewHeaderDayStyle);
      }
  ...
  getColorForDayInMonthView(DateTime today,TextStyle textStyle){
    TextStyle todayStyle=textStyle;
    if(today.weekday==6){
      todayStyle= todayStyle.copyWith(color: Color(0xff0800FF));
    }
    if(today.weekday==7){
      todayStyle= todayStyle.copyWith(color: Color(0xffFF0000));
    }
    return todayStyle;
  }
  ...
  void _addViewHeaderForTimeSlotViews(
    ...
    for (int i = 0; i < visibleDatesLength; i++) {
      ...
      _dayTextPainter.paint(
          canvas, Offset(xPosition + dayXPosition, yPosition-16));

      if (isToday) {
        _drawTodayCircle(
            canvas,
            xPosition + dateXPosition,
            yPosition + topPadding + _dayTextPainter.height + inBetweenPadding-10,
            _dateTextPainter);
      }
...
class _TimeRulerView extends CustomPainter {
  ...
  void _drawTimeLabels(Canvas canvas, Size size, DateTime date, double hour,
    ...
    for (int i = isTimelineView ? 0 : 1;
        i <= (isTimelineView ? horizontalLinesCount - 1 : horizontalLinesCount+1);
        i++) {


      if(i==1){
        log("Horizontal Count ${horizontalLinesCount+1}");
        log("in the loop 1");
        ...
        final String time =
        DateFormat(timeSlotViewSettings.timeFormat, locale).format(date.subtract(Duration(hours: 1)));
